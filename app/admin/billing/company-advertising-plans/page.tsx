  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <Link href="/admin/billing/company-advertising-plans/new" className="rounded bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground">Create Plan</Link>
      </div>
          ...
          <tbody className="divide-y divide-border">
            {plans.map((plan, index) => (
              <tr key={plan.id}>
                ...
                <td className="px-4 py-3">
                  <Link href={`/admin/billing/company-advertising-plans/${plan.id}`} className="rounded border px-3 py-1.5 text-xs font-semibold text-foreground">Manage</Link>
                </td>
              </tr>
            ))}
          </tbody>
