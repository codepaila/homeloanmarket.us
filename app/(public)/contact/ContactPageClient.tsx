...
function formatAddress(s: SiteSettings): { street: string; cityStateZip: string } | null {
  const parts = [s.contactStreet, s.contactCity, s.contactState, s.contactZip, s.contactCountry].filter(Boolean)
  if (parts.length === 0) return null

  const street = s.contactStreet || ''
  const locality = [s.contactCity, s.contactState, s.contactZip].filter(Boolean).join(', ')
  return { street, cityStateZip: locality || s.contactCountry }
}
...
export function ContactPageClient({ settings }: { settings: SiteSettings }) {
  ...
  const hasPhone = Boolean(settings.contactPhone)
  const hasEmail = Boolean(settings.contactEmail)
  const hasBusinessHours = Boolean(settings.contactBusinessHours)
                ...
                {hasBusinessHours && (
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground whitespace-pre-line">{settings.contactBusinessHours}</p>
                    </div>
                  </div>
                )}
              </div>
