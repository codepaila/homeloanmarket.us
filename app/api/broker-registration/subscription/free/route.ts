...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (current?.status === 'ACTIVE' && current.plan === 'FEATURED') throw new Error('An existing FEATURED subscription must be managed before selecting FREE')
    const subscription = await prisma.brokerRegistrationSubscription.upsert({
      where: { registrationId: user.brokerRegistration!.id },
      update: { plan: 'FREE', status: 'ACTIVE', isActive: true, startDate: new Date(), endDate: null, stripeSubId: null },
      create: { registrationId: user.brokerRegistration!.id, plan: 'FREE', status: 'ACTIVE', isActive: true, startDate: new Date() },
    })
    await prisma.brokerRegistration.update({ where: { id: user.brokerRegistration!.id }, data: { status: 'ONBOARDING_IN_PROGRESS' } })
    return subscription
    })
