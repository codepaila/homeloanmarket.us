// lib/company-subscription-email-state.ts
//
// Pure helpers for the company subscription purchase email durable state
...
export function isClaimEligible(status: CompanySubscriptionEmailStatus | null | undefined, leaseExpiresAt: Date | null | undefined, now: Date): boolean {
...
export function claimEligibleWhere(idempotencyKey: string, now: Date): Prisma.CompanySubscriptionEmailLogWhereInput {
  return {
    idempotencyKey,
    OR: [
      { status: { in: ['PENDING', 'FAILED'] } as Prisma.EnumCompanySubscriptionEmailStatusFilter },
