...
export class AdvertisementService {
  ...
  private static async validateCreativeAssignments(placement: string, assignments: { mediaAssetId: string; format: AdvertisementFormat }[]) {
    if (placement === 'BROKER_LISTING_LOCAL' && !assignments.some((assignment) => assignment.format === 'SQUARE' || assignment.format === 'BANNER' || assignment.format === 'WIDE_RECTANGLE')) {
      throw new Error('BROKER_LISTING_LOCAL advertisements require a SQUARE, BANNER, or WIDE_RECTANGLE creative')
    }
